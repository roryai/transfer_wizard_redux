...
def test_generates_path_including_year_and_quarter():
    ...
    generated_path = generator.target_path(source_filepath, target_dir, filename)
...
def test_creates_year_and_quarter_target_directories():
    ...
    desireded_target_path = target_dir + '2023/Q2/test_file.txt'

    assert not os.path.isdir(desired_target_dir)

    generator.create_directory(desireded_target_path)
...
def test_creates_quarter_target_directory_if_year_directory_already_exists():
    ...
    desired_target_path = target_dir + '2023/Q2/test_file.txt'

    assert not os.path.isdir(desired_target_dir_year)

    os.makedirs(desired_target_dir_year)

    generator.create_directory(desired_target_path)

...
@pytest.fixture(autouse=True)
def mock_capture_date(monkeypatch):
    def mock_date(self, source_filepath):
        return datetime.strptime('1 October 2023', "%d %B %Y").date()

    monkeypatch.setattr(CaptureDateIdentifier, "capture_date", mock_date)


def generate_filepath(source_filepath):
    return FilepathGenerator(source_filepath, destination_root_directory).generate_destination_filepath()


def test_generates_filepath():
...
def test_adds_suffix_to_filename_if_existing_file_has_same_name_and_different_size():
    filename, source_filepath, destination_directory, _ = create_test_files(create_destination_file=True)
